...
本文以 Django项目为例子，叙述在处理遗留代码过程中所使用的多项技术。阅读代码库通常是容易被低估的一个基本技能。在实践过程中，我们应当尽可能使用原有的可以运行良好的代码，而不是重复地发明轮子。另外测试案例也是代码编程一个非常重要的组成部分。

<!-- more -->

## A 概述

参加一个项目的开发是一个令人激动人心的事情，在此过程中你可以了解和使用到一些强大的工具和前沿的技术。然而常常接收的一个已经存在的，甚至早期远古时期的项目。当然，Django 发行于 2005 年，可能还没有那么久远。使用早期 Django 项目可能是完全不同的，源码和文档可能是不够的。

## B Django 版本

使用哪个版本的 Django 是一个重要的基础步骤。随着 Django 不断发展和完善，一些概念和规范可能有所变化，比如  *默认目录结构* 和 *最佳实践* 。

### 依赖文件

一般来说，每个项目的根目录会有一个 *requirements.txt* 或者 *setup.py* 的文件，表明了这个项目的 Django 版本是多少，通常是以下的格式：

```
django=1.11.2
```

使用 “=” 描述的依赖，是一个非常精确的版本。与此相对应的是类似 `django>=1.8` 的形式 ，表示是一个范围，实际安装时总是以符合该范围的最新版本为准。

> **延伸** 固定依赖库的版本是一个最佳实践，这样会减少一些意想不到的错误，构建过程具有确定性。[pipenv](https://docs.pipenv.org/) 是一个 Python 项目开发构建工具，使用  Pipfile.lock 文件产生确定性的构建过程。

在现实的项目则是没有 *requirements.txt* 文件或 *setup.py* 文件，这时候你可能需要尝试其他方式来确定它的 Django 版本。

### 虚拟环境

在大多情况下，Django 项目可能部署在一个虚拟环境或者携带有虚拟环境，事实上，这种做法也是一个最佳实践额规则。当你位于一个虚拟环境时，可以按照下面的步骤查询 Django 版本。
...
如果上述的方法全部失效的话，需要根据项目的一些特性并结合版本发布日志可以大致确定版本范围，某些情况下甚至可以精确到确定的版本。
...
| ------- | ------ |
| 1.4 LTS | 新的默认项目文件布局 |
| 1.5 | `AUTH_PROFILE_MODULE` 被移除，使用 `AUTH_USER_MODEL` |
| 1.7 | 支持数据库迁移，可以使用 `migrate` 命令 |
| 1.8 LTS | 开始支持 Jinja2 模板引擎。|
| 2.0 | 不再支持 Python2 |

一般来说，这些特性都可以在项目的 `settings.py` 模块文件中体现出来。使用 `django-admin startproject` 命令生成的项目，在 `settings.py` 的顶部含有类似下面的注释文字，清楚地记载了该项目所使用的具体版本。
...
Django 使用的是 MTV 的架构模式，在项目中，路由 是一个很好的入口点，包含了从每个请求到响应的映射。代码上可能为一个模块文件(Module)，复杂的情况下也可能是一个包(Package)。但无论何种情况，总是存在 *根路由* ，并且定义在项目配置模块中。
